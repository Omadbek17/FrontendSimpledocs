...
const Register = ({ onRegisterSuccess }) => {
  ...
  const styles = {
    ...
    input: {
      width: "100%", padding: 12, marginBottom: 20, borderRadius: 6,
      border: `1.5px solid ${isDarkMode ? "#666" : "#bbb"}`,
      backgroundColor: isDarkMode ? "#2a2a2a" : "#fff",
      color: isDarkMode ? "#fff" : "#000", fontSize: 16, boxSizing: "border-box",
      transition: "border-color 0.3s ease, background-color 0.3s ease",
    },
    focus: { borderColor: "#007bff", outline: "none" },
    button: {
      width: "100%", padding: 14, borderRadius: 6, border: "none",
      backgroundColor: "#007bff", color: "#fff", fontSize: 18, cursor: "pointer",
      transition: "background-color 0.3s ease",
    },
    disabledButton: { backgroundColor: "#a0a0a0", cursor: "not-allowed" },
    message: {
      marginTop: 20, fontWeight: "600", textAlign: "center", padding: "10px 15px",
      borderRadius: 6, fontSize: 16, userSelect: "none",
    },
    success: {
    ...
    <div style={styles.form}>
      ...
      <form onSubmit={handleRegister}>
        <input
          ...
          style={{ ...styles.input, ...(isFocused.username ? styles.focus : {}) }}
        ...
        <input
          ...
          style={{ ...styles.input, ...(isFocused.password ? styles.focus : {}) }}
      ...
      {message && (
        <p style={{ ...styles.message, ...(isSuccess ? styles.success : styles.error) }}>
          {message}
        </p>
      )}
    </div>
