...
class ShareDocumentView(APIView):
    ...
    def post(self, request, pk):
        ...
        return Response({"message": f"Document shared with {username}."}, status=status.HTTP_200_OK)


# ----------------------------------
# ✅ Register endpoint (open to all)
# ----------------------------------
...
def register(request):
    ...
    if len(password) < 4:
        return Response(
            {"success": False, "error": "Password too short (min 4 characters)."},
            status=status.HTTP_400_BAD_REQUEST,
        )
